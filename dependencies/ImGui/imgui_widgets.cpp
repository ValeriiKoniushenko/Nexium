    ...
    // Clang/GCC warnings with -Weverything
            ...
            #pragma clang diagnostic ignored                                                       \
                "-Wunknown-warning-option" // warning: unknown warning group 'xxx' // not all
                                           // warnings are known by all Clang versions and they tend
                                           // to be rename-happy.. so ignoring warnings triggers new
        ...
        #pragma clang diagnostic ignored "-Wformat" // warning: format specifies type 'int' but the
        ...
        #pragma clang diagnostic ignored                                                           \
            "-Wunused-macros" // warning: macro is not used                                // we
                              // define snprintf/vsnprintf on Windows so they are available, but not
                              // always used.
        #pragma clang diagnostic ignored                                                           \
        ...
        #pragma clang diagnostic ignored                                                           \
            "-Wdouble-promotion" // warning: implicit conversion from 'float' to 'double' when
                                 // passing argument to function  // using printf() is a misery with
                                 // this as C++ va_arg ellipsis changes float to double.
        #pragma clang diagnostic ignored                                                           \
        ...
        #pragma GCC diagnostic ignored                                                             \
            "-Wformat" // warning: format '%p' expects argument of type 'int'/'void*', but argument
                       // X has type 'unsigned int'/'ImGuiWindow*'
        #pragma GCC diagnostic ignored "-Wformat-nonliteral" // warning: format not a string
...
static glm::vec2 InputTextCalcTextSize(ImGuiContext* ctx, const char* text_begin,
                                       const char* text_end, const char** remaining = NULL,
                                       glm::vec2* out_offset = NULL, bool stop_on_new_line = false);
...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    const glm::vec2 text_pos(window->DC.CursorPos.x,
...
void ImGui::TextAlignedV(float align_x, float size_x, const char* fmt, va_list args)
{
    ...
    glm::vec2 pos(window->DC.CursorPos.x,
                  window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
    ...
    const ImRect total_bb(
        pos,
        pos
            + glm::vec2(w + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f),
    ...
    RenderTextClipped(value_bb.Min + style.FramePadding, value_bb.Max, value_text_begin,
                      value_text_end, &value_size, glm::vec2(0.0f, 0.0f));
    if (label_size.x > 0.0f)
    {
        RenderText(glm::vec2(value_bb.Max.x + style.ItemInnerSpacing.x,
...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    const glm::vec2 total_size = glm::vec2(
    ...
    RenderBullet(window->DrawList,
                 bb.Min + glm::vec2(style.FramePadding.x + g.FontSize * 0.5f, g.FontSize * 0.5f),
                 text_col);
    RenderText(bb.Min + glm::vec2(g.FontSize + style.FramePadding.x * 2, 0.0f), text_begin,
               text_end, false);
}
                           ...
                           ImGuiButtonFlags flags)
{
    ...
    // where holding the button pressed for a long time while drag a payload item triggers the
    // button.
        ...
        if (mods_ok)
        {
            ...
            if (flags & ImGuiButtonFlags_PressedOnRelease)
            {
                if (mouse_button_released != -1)
                {
                    ...
                    if (!(flags & ImGuiButtonFlags_NoNavFocus))
                    {
                        SetFocusID(id, window); // FIXME: Lack of FocusWindow() call here is
                     ...
                     const glm::vec2* preCalcLabelSize)
{
    ...
    glm::vec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 2.0f,
...
bool ImGui::ArrowButtonEx(const char* str_id, ImGuiDir dir, glm::vec2 size, ImGuiButtonFlags flags)
{
    ...
    RenderArrow(window->DrawList,
                bb.Min
                    + glm::vec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f),
...
bool ImGui::CloseButton(ImGuiID id, glm::vec2 pos)
{
    ...
    window->DrawList->AddLine(cross_center + glm::vec2(+cross_extent, +cross_extent),
                              cross_center + glm::vec2(-cross_extent, -cross_extent), cross_col,
                              cross_thickness);
    window->DrawList->AddLine(cross_center + glm::vec2(+cross_extent, -cross_extent),
                              cross_center + glm::vec2(-cross_extent, +cross_extent), cross_col,
                              cross_thickness);
...
bool ImGui::CollapseButton(ImGuiID id, glm::vec2 pos, ImGuiDockNode* dock_node)
{
    ...
    {
        StartMouseMovingWindowOrNode(window, dock_node,
                        ...
                        ImDrawFlags draw_rounding_flags)
{
    ...
    ImRect bb = bb_frame;
    bb.Expand(glm::vec2(-ImClamp(IM_TRUNC((bb_frame_width - 2.0f) * 0.5f), 0.0f, 3.0f),
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        else
        {
                ...
                && held_dir == g.ScrollbarSeekMode)
            {
                ...
                *p_scroll_v = ImClamp(*p_scroll_v + (ImS64)(page_dir * size_visible_v), (ImS64)0,
                                      scroll_max);
            }
        }
                        ...
                        glm::vec2 uv1, const glm::vec4& bg_col, const glm::vec4& tint_col)
{
    ...
    {
        window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_Border), 0.0f,
                                  ImDrawFlags_None, g.Style.ImageBorderSize);
    }
    if (bg_col.w > 0.0f)
    {
                        ...
                        const glm::vec4& tint_col)
{
    ...
    // - old ImageButton() used ImTextureID as item id (created issue with multiple buttons with
    // same image, transient texture id values, opaque computation of ID)
    ...
    tint_col)
    {
        ...
        if (frame_padding >= 0)
            PushStyleVar(ImGuiStyleVar_FramePadding, glm::vec2((float)frame_padding,
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (is_visible)
    {
        ...
        if (mixed_value)
        {
            ...
            glm::vec2 pad(ImMax(1.0f, IM_TRUNC(square_sz / 3.6f)),
        ...
        else if (*v)
        {
            ...
            RenderCheckMark(window->DrawList, check_bb.Min + glm::vec2(pad, pad), check_col,
                            square_sz - pad * 2.0f);
        }
    }
    const glm::vec2 label_pos = glm::vec2(check_bb.Max.x + style.ItemInnerSpacing.x,
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    {
        window->DrawList->AddCircle(center + glm::vec2(1, 1), radius,
    ...
    glm::vec2 label_pos = glm::vec2(check_bb.Max.x + style.ItemInnerSpacing.x,
                                    check_bb.Min.y + style.FramePadding.y);
    if (g.LogEnabled)
    {
        ...
        if (overlay_size.x > 0.0f)
        {
            ...
            RenderTextClipped(
                glm::vec2(
                    ...
                    bb.Min.y),
                bb.Max, overlay, NULL, &overlay_size, glm::vec2(0.0f, 0.5f), &bb);
        }
    }
}
...
void ImGui::Bullet()
{
    ...
    const ImRect bb(window->DC.CursorPos,
                    window->DC.CursorPos + glm::vec2(g.FontSize, line_height));
    ItemSize(bb);
    if (!ItemAdd(bb, 0))
    {
    ...
    ImU32 text_col = GetColorU32(ImGuiCol_Text);
    RenderBullet(window->DrawList,
                 bb.Min + glm::vec2(style.FramePadding.x + g.FontSize * 0.5f, line_height * 0.5f),
                 text_col);
...
bool ImGui::TextLink(const char* label)
{
    ...
    window->DrawList->AddLine(glm::vec2(bb.Min.x, line_y), glm::vec2(bb.Max.x, line_y),
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    if (flags & ImGuiSeparatorFlags_Vertical)
    {
        ...
        const ImRect bb(glm::vec2(window->DC.CursorPos.x, y1),
                        glm::vec2(window->DC.CursorPos.x + thickness, y2));
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        const ImRect bb(glm::vec2(x1, window->DC.CursorPos.y),
                        glm::vec2(x2, window->DC.CursorPos.y + thickness));
...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label, const char* label_end, float extra_w)
{
    ...
    const glm::vec2 min_size(label_size.x + extra_w + padding.x * 2.0f,
    ...
    if (label_size.x > 0.0f)
    {
        ...
        {
            window->DrawList->AddLine(glm::vec2(sep1_x1, seps_y), glm::vec2(sep1_x2, seps_y),
                                      separator_col, separator_thickness);
        }
        if (sep2_x2 > sep2_x1 && separator_thickness > 0.0f)
        {
            window->DrawList->AddLine(glm::vec2(sep2_x1, seps_y), glm::vec2(sep2_x2, seps_y),
                                      separator_col, separator_thickness);
        }
        if (g.LogEnabled)
        {
        ...
        RenderTextEllipsis(window->DrawList, label_pos,
                           glm::vec2(bb.Max.x, bb.Max.y + style.ItemSpacing.y), bb.Max.x, label,
    ...
    else
    {
        ...
        {
            window->DrawList->AddLine(glm::vec2(sep1_x1, seps_y), glm::vec2(sep2_x2, seps_y),
                                      separator_col, separator_thickness);
        }
    }
}
                             ...
                             float hover_visibility_delay, ImU32 bg_col)
{
    ...
    bool hovered, held;
    ImRect bb_interact = bb;
    bb_interact.Expand(axis == ImGuiAxis_Y ? glm::vec2(0.0f, hover_extend)
                                           : glm::vec2(hover_extend, 0.0f));
    ButtonBehavior(bb_interact, id, &hovered, &held, button_flags);
    if (hovered)
    {
        g.LastItemData.StatusFlags
            |= ImGuiItemStatusFlags_HoveredRect; // for IsItemHovered(), because bb_interact is
                                                 // larger than bb
    }
    ...
    if (held)
    {
        ...
        if (mouse_delta != 0.0f)
        {
            ...
            *size2 = ImMax(*size2 - mouse_delta, min_size2);
            bb_render.Translate((axis == ImGuiAxis_X) ? glm::vec2(mouse_delta, 0.0f)
                                                      : glm::vec2(0.0f, mouse_delta));
            MarkItemEdited(id);
        }
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos,
                    window->DC.CursorPos
                        + glm::vec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(
        bb.Min,
        bb.Max
            + glm::vec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f,
                        0.0f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
    {
        window->DrawList->AddRectFilled(
            bb.Min, glm::vec2(value_x2, bb.Max.y), frame_col, style.FrameRounding,
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        window->DrawList->AddRectFilled(
            glm::vec2(value_x2, bb.Min.y), bb.Max, bg_col, style.FrameRounding,
            (w <= arrow_size) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);
        if (value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x)
        {
            RenderArrow(window->DrawList,
                        glm::vec2(value_x2 + style.FramePadding.y, bb.Min.y + style.FramePadding.y),
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        ...
        RenderTextClipped(bb.Min + style.FramePadding, glm::vec2(value_x2, bb.Max.y), preview_value,
                          NULL, NULL);
    }
    if (label_size.x > 0)
    {
        RenderText(glm::vec2(bb.Max.x + style.ItemInnerSpacing.x, bb.Min.y + style.FramePadding.y),
                   label);
    }
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
    {
        if (popup_window->WasActive)
        {
            ...
            glm::vec2 pos = FindBestWindowPosForPopupEx(
    ...
    IM_ASSERT(g.LastItemData.Rect.Min.x == preview_data->PreviewRect.Min.x
              && g.LastItemData.Rect.Min.y
                     == preview_data->PreviewRect.Min
                            .y); // Didn't call after BeginCombo/EndCombo block or forgot to pass
                  ...
                  int popup_max_height_in_items)
{
        ...
        && !(g.NextWindowData.HasFlags & ImGuiNextWindowDataFlags_HasSizeConstraint))
    {
        SetNextWindowSizeConstraints(
            glm::vec2(0, 0),
            glm::vec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
                       ...
                       ImGuiSliderFlags flags)
{
    ...
    if (label_size.x > 0.0f)
    {
        RenderText(glm::vec2(frame_bb.Max.x + style.ItemInnerSpacing.x,
                ...
                = (-(float)v_min)
                  / ((float)v_max
                     - (float)v_min); // The zero point in parametric space.  There's an argument we
                                      // should take the logarithmic nature into account when
                                      // calculating this, but for now this should do (and the most
                          ...
                          ImGuiSliderFlags flags)
{
    ...
    const char* value_buf_end
        = value_buf
          + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    RenderTextClipped(glm::vec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y),
                      frame_bb.Max, value_buf, value_buf_end, NULL, glm::vec2(0.5f, 0.0f));
    if (label_size.x > 0.0f)
    {
        RenderText(glm::vec2(frame_bb.Max.x + style.ItemInnerSpacing.x,
                             frame_bb.Min.y + style.FramePadding.y),
                            ...
                            const void* p_clamp_min, const void* p_clamp_max)
{
    ...
    g.LastItemData.ItemFlags
        |= ImGuiItemFlags_NoMarkEdited; // Because TempInputText() uses
                                        // ImGuiInputTextFlags_MergedItem it doesn't submit a new
                        ...
                        ImGuiInputTextFlags flags)
{
    ...
    IM_ASSERT((flags & ImGuiInputTextFlags_EnterReturnsTrue)
              == 0); // Not supported by InputScalar(). Please open an issue if you this would be
                      ...
                      ImGuiInputTextCallback callback, void* user_data)
{
    ...
    return InputTextEx(label, NULL, buf, (int)buf_size, glm::vec2(0, 0), flags, callback,
                       user_data);
}
                              ...
                              void* user_data)
{
    ...
    return InputTextEx(label, hint, buf, (int)buf_size, glm::vec2(0, 0), flags, callback,
                       user_data);
}
...
static glm::vec2 InputTextCalcTextSize(ImGuiContext* ctx, const char* text_begin,
                                       ...
                                       glm::vec2* out_offset, bool stop_on_new_line)
{
    ImGuiContext& g = *ctx;
    // ImFont* font = g.Font;
...
namespace ImStb
{
                                       ...
                                       int line_start_idx)
    {
        ...
        const glm::vec2 size = InputTextCalcTextSize(
            obj->Ctx, text + line_start_idx, text + obj->TextLen, &text_remaining, NULL, true);
        r->x0 = 0.0f;
        r->x1 = size.x;
    ...
    if (c < 0x20)
    {
        ...
        pass |= (c == '\n')
                && (flags & ImGuiInputTextFlags_Multiline)
                       != 0; // Note that an Enter KEY will emit \r and be ignored (we poll for KEY
                             // in InputText() code)
                        ...
                        const std::function<void(const char*)>& onApplyNewTextCallback)
{
    ...
    const ImGuiID id = window->GetID(label);
    const glm::vec2 label_size = CalcTextSize(label, NULL, true);
    const glm::vec2 frame_size = CalcItemSize(
    ...
    if (is_multiline)
    {
        ...
        PushStyleVar(
            ...
            glm::vec2(0, 0)); // Ensure no clip rect so mouse hover can reach FramePadding edges
    ...
    if (init_reload_from_user_buf)
    {
        ...
        state->TextA.resize(buf_size + 1); // we use +1 to make sure that .Data is always pointing
    ...
    else if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        if (!is_readonly)
        {
            state->TextA.resize(buf_size + 1); // we use +1 to make sure that .Data is always
    ...
    if (g.ActiveId == id)
    {
        ...
        // FIXME: The reason we don't use Shortcut() is we would need a routing flag to specify
        // multiple mods, or to all mods combination into individual shortcuts.
        ...
        // if (is_readonly && state != NULL)
        //     state->TextA.clear(); // Uncomment to facilitate debugging, but we otherwise prefer
        //     to keep/amortize th allocation.
    }
    if (state != NULL)
    {
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_wordmove_key_down
            = is_osx ? io.KeyAlt : io.KeyCtrl; // OS X style: Text editing cursor movement using Alt
                                               // instead of Ctrl
        ...
        // operations for other code (e.g. calling window trying to use CTRL+A and CTRL+B: former
        ...
        else if (is_cut || is_copy)
        {
            ...
            {
                // SetClipboardText() only takes null terminated strings + state->TextSrc may point
                // to read-only user buffer, so we need to make a copy.
    ...
    if (g.ActiveId == id)
    {
        ...
        // ActiveId, even though strictly speaking it wasn't modified on this frame. Should mark
        // dirty state from the stb_textedit callbacks. If we do that, need to ensure that as
        // special case, 'validated == true' also writes back. This also allows the user to use
        // InputText() without maintaining any user-side storage. (please note that if you use this
        // property along ImGuiInputTextFlags_CallbackResize you can end up with your temporary
        // string object unnecessarily allocating once a frame, either store your string data,
                ...
                if (event_flag)
                {
                    ...
                    if (buf_dirty)
                    {
                        ...
                        IM_ASSERT(
                            ...
                            == (int)ImStrlen(callback_data.Buf)); // You need to maintain BufTextLen
    ...
    if (apply_new_text != NULL)
    {
        ...
        if (onApplyNewTextCallback)
        {
            onApplyNewTextCallback(apply_new_text);
        }
        if (is_resizable)
        {
            ImGuiInputTextCallbackData callback_data;
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            for (const char* p = text_selected_begin; p < text_selected_end;)
            {
                ...
                else
                {
                    ...
                    ImRect rect(rect_pos + glm::vec2(0.0f, bg_offy_up - g.FontSize),
                                rect_pos + glm::vec2(rect_size.x, bg_offy_dn));
        ...
        if (render_cursor)
        {
            ...
            // can cover our cursor. Bit of an extra nicety.) This is required for some backends
            // (SDL3) to start emitting character/text inputs. As per #6341, make sure we don't set
            // that on the deactivating frame.
            if (!is_readonly && g.ActiveId == id)
            {
                ...
                ime_data->InputPos
                    = glm::vec2(cursor_screen_pos.x - 1.0f, cursor_screen_pos.y - g.FontSize);
    ...
    else
    {
        ...
        if (is_multiline)
        {
            text_size = glm::vec2(inner_size.x,
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            if (flags & ImGuiInputTextFlags_ElideLeft)
            {
                draw_pos.x = ImMin(draw_pos.x, frame_bb.Max.x - CalcTextSize(buf_display, NULL).x
                                                   - style.FramePadding.x);
            }

            const glm::vec2 draw_scroll = /*state ? glm::vec2(state->Scroll.x, 0.0f) :*/ glm::vec2(
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state)
{
    ...
    Text("CurLenA: %d, Cursor: %d, Selection: %d..%d", state->TextLen, stb_state->cursor,
    ...
    if (BeginChild("undopoints", glm::vec2(0.0f, GetTextLineHeight() * 10),
                   ImGuiChildFlags_Borders | ImGuiChildFlags_ResizeY)) // Visualize undo state
    {
        ...
        for (int n = 0; n < IMSTB_TEXTEDIT_UNDOSTATECOUNT; n++)
        {
            ...
            Text("%c [%02d] where %03d, insert %03d, delete %03d, char_storage %03d \"%.*s\"",
                 ...
                 undo_rec->delete_length, undo_rec->char_storage, buf_preview_len, buf_preview_str);
            if (undo_rec_type == ' ')
            {
                EndDisabled();
            }
...
static void ColorEditRestoreH(const float* col, float* H)
{
    ...
    if (g.ColorEditSavedID != g.ColorEditCurrentID
        || g.ColorEditSavedColor
               != ImGui::ColorConvertFloat4ToU32(glm::vec4(col[0], col[1], col[2], 0)))
    {
        return;
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
        ...
        if (ColorButton("##ColorButton", col_v4, flags))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                ...
                SetNextWindowPos(g.LastItemData.Rect.GetBL()
                                 + glm::vec2(0.0f, style.ItemSpacing.y));
            }
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
        {
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        window->DC.CursorPos.x
            = pos.x
              + ((flags & ImGuiColorEditFlags_NoInputs) ? w_button
...
static void RenderArrowsForVerticalBar(ImDrawList* draw_list, glm::vec2 pos, glm::vec2 half_sz,
                                       float bar_w, float alpha)
{
    ImU32 alpha8 = IM_F32_TO_INT8_SAT(alpha);
    ImGui::RenderArrowPointingAt(draw_list, glm::vec2(pos.x + half_sz.x + 1, pos.y),
                                 glm::vec2(half_sz.x + 2, half_sz.y + 1), ImGuiDir_Right,
                                 IM_COL32(0, 0, 0, alpha8));
    ImGui::RenderArrowPointingAt(draw_list, glm::vec2(pos.x + half_sz.x, pos.y), half_sz,
                                 ImGuiDir_Right, IM_COL32(255, 255, 255, alpha8));
    ImGui::RenderArrowPointingAt(draw_list, glm::vec2(pos.x + bar_w - half_sz.x - 1, pos.y),
                                 glm::vec2(half_sz.x + 2, half_sz.y + 1), ImGuiDir_Left,
                                 IM_COL32(0, 0, 0, alpha8));
    ImGui::RenderArrowPointingAt(draw_list, glm::vec2(pos.x + bar_w - half_sz.x, pos.y), half_sz,
                         ...
                         const float* ref_col)
{
    ...
    glm::vec2 wheel_center(picker_pos.x + (sv_picker_size + bars_width) * 0.5f,
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        InvisibleButton("hsv", glm::vec2(sv_picker_size + style.ItemInnerSpacing.x + bars_width,
                                         sv_picker_size));
        if (IsItemActive() && !is_readonly)
        {
                                        ...
                                        ImRotate(initial_off, cos_hue_angle, sin_hue_angle)))
            {
                ...
                glm::vec2 current_off_unrotated
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        glm::vec4 col_v4(col[0], col[1], col[2],
        ...
        ColorButton("##current", col_v4, (flags & sub_flags_to_forward),
                    glm::vec2(square_sz * 3, square_sz * 2));
        if (ref_col != NULL)
        {
            Text("Original");
            glm::vec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2],
    ...
    glm::vec4 hue_color_f(1, 1, 1, style.Alpha);
    ColorConvertHSVtoRGB(H, 1, 1, hue_color_f.x, hue_color_f.y, hue_color_f.z);
    ImU32 hue_color32 = ColorConvertFloat4ToU32(hue_color_f);
    ImU32 user_col32_striped_of_alpha = ColorConvertFloat4ToU32(glm::vec4(
        R, G, B,
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        for (int n = 0; n < 6; n++)
        {
            ...
            glm::vec2 gradient_p0(wheel_center.x + ImCos(a0) * wheel_r_inner,
                                  wheel_center.y + ImSin(a0) * wheel_r_inner);
            glm::vec2 gradient_p1(wheel_center.x + ImCos(a1) * wheel_r_inner,
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        draw_list->AddRectFilledMultiColor(picker_pos,
                                           picker_pos + glm::vec2(sv_picker_size, sv_picker_size),
        ...
        for (int i = 0; i < 6; ++i)
        {
            draw_list->AddRectFilledMultiColor(
                glm::vec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6)),
                glm::vec2(bar0_pos_x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6)),
                col_hues[i], col_hues[i], col_hues[i + 1], col_hues[i + 1]);
        }
        float bar0_line_y = IM_ROUND(picker_pos.y + H * sv_picker_size);
        RenderFrameBorder(glm::vec2(bar0_pos_x, picker_pos.y),
                          glm::vec2(bar0_pos_x + bars_width, picker_pos.y + sv_picker_size), 0.0f);
        RenderArrowsForVerticalBar(draw_list, glm::vec2(bar0_pos_x - 1, bar0_line_y),
                                   glm::vec2(bars_triangles_half_sz + 1, bars_triangles_half_sz),
    ...
    if (alpha_bar)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(draw_list, bar1_bb.Min, bar1_bb.Max, 0,
                                             bar1_bb.GetWidth() / 2.0f, glm::vec2(0.0f, 0.0f));
        ...
        RenderArrowsForVerticalBar(draw_list, glm::vec2(bar1_pos_x - 1, bar1_line_y),
                                   glm::vec2(bars_triangles_half_sz + 1, bars_triangles_half_sz),
                                   bars_width + 2.0f, style.Alpha);
    }
                        ...
                        glm::vec2 size_arg)
{
    ...
    const float default_size = GetFrameHeight();
    const glm::vec2 size(size_arg.x == 0.0f ? default_size : size_arg.x,
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        if ((flags & ImGuiColorEditFlags_AlphaNoBg) == 0)
        {
            RenderColorRectWithAlphaCheckerboard(
                window->DrawList, glm::vec2(bb_inner.Min.x + grid_step, bb_inner.Min.y),
                bb_inner.Max, GetColorU32(col_rgb), grid_step, glm::vec2(-grid_step + off, off),
                rounding, ImDrawFlags_RoundCornersRight);
        }
        else
        {
            window->DrawList->AddRectFilled(glm::vec2(bb_inner.Min.x + grid_step, bb_inner.Min.y),
                                            bb_inner.Max, GetColorU32(col_rgb), rounding,
                                            ImDrawFlags_RoundCornersRight);
        }
        window->DrawList->AddRectFilled(bb_inner.Min, glm::vec2(mid_x, bb_inner.Max.y),
    ...
    else
    {
        ...
        glm::vec4 col_source
            = (flags & ImGuiColorEditFlags_AlphaOpaque) ? col_rgb_without_alpha : col_rgb;
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    glm::vec2 sz(g.FontSize * 3 + g.Style.FramePadding.y * 2,
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_picker)
    {
        glm::vec2 picker_size(
            g.FontSize * 8, ImMax(g.FontSize * 8 - (GetFrameHeight() + g.Style.ItemInnerSpacing.x),
                             ...
                             const char* label_end)
{
    ...
    glm::vec2 text_pos(window->DC.CursorPos.x + text_offset_x,
    ...
    // Render
    {
        ...
        if (display_frame)
        {
            ...
            if (flags & ImGuiTreeNodeFlags_Bullet)
            {
                RenderBullet(
                    window->DrawList,
                    glm::vec2(text_pos.x - text_offset_x * 0.60f, text_pos.y + g.FontSize * 0.5f),
                    text_col);
            }
            else if (!is_leaf)
            {
                RenderArrow(window->DrawList,
                            glm::vec2(text_pos.x - text_offset_x + padding.x, text_pos.y), text_col,
        ...
        else
        {
            ...
            if (flags & ImGuiTreeNodeFlags_Bullet)
            {
                RenderBullet(
                    window->DrawList,
                    glm::vec2(text_pos.x - text_offset_x * 0.5f, text_pos.y + g.FontSize * 0.5f),
                    text_col);
            }
            else if (!is_leaf)
            {
                RenderArrow(window->DrawList,
                            glm::vec2(text_pos.x - text_offset_x + padding.x,
        ...
        if (draw_tree_lines)
        {
            TreeNodeDrawLineToChildNode(
                glm::vec2(text_pos.x - text_offset_x + padding.x, text_pos.y + g.FontSize * 0.5f));
        }
...
void ImGui::TreeNodeDrawLineToChildNode(glm::vec2 target_pos)
{
    ...
    {
        window->DrawList->AddLine(glm::vec2(x1, y), glm::vec2(x2, y),
                                  GetColorU32(ImGuiCol_TreeLines), g.Style.TreeLinesSize);
    }
}
...
void ImGui::TreeNodeDrawLineToTreePop(const ImGuiTreeNodeStackData* data)
{
    ...
    window->DrawList->AddLine(glm::vec2(x, y1), glm::vec2(x, y2), GetColorU32(ImGuiCol_TreeLines),
                              g.Style.TreeLinesSize);
    if (data->DrawLinesTableColumn != -1)
    {
                       ...
                       glm::vec2 size_arg)
{
    ...
    glm::vec2 size(size_arg.x != 0.0f ? size_arg.x : label_size.x,
    ...
    if (is_visible)
    {
        ...
        {
            // Between 1.91.0 and 1.91.4 we made selected Selectable use an arbitrary lerp between
            // _Header and _HeaderHovered. Removed that now. (#8106)
    ...
    if (is_visible)
    {
        RenderTextClipped(pos,
                          glm::vec2(ImMin(pos.x + size.x, window->WorkRect.Max.x), pos.y + size.y),
...
ImGuiTypingSelectRequest* ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags)
{
    ...
    if (flags & ImGuiTypingSelectFlags_AllowSingleCharMode)
    {
        ...
        data->SingleCharModeLock
            |= (single_char_count
                >= TYPING_SELECT_SINGLE_CHAR_COUNT_FOR_LOCK); // From now on we stop search matching
                                                              // to lock to single char mode.
    }
    ...
    for (const ImGuiSelectionRequest& req : io->Requests)
    {
        ...
        if (req.Type == ImGuiSelectionRequestType_SetRange)
        {
            IMGUI_DEBUG_LOG_SELECTION(
                "[selection] %s: Request: SetRange %" IM_PRId64 "..%" IM_PRId64 " (0x%" IM_PRIX64
...
static ImRect CalcScopeRect(ImGuiMultiSelectTempData* ms, ImGuiWindow* window)
{
    ...
    else
    {
        ...
        scope_rect.Min
            = ImMin(scope_rect.Min + glm::vec2(window->DecoInnerSizeX1, window->DecoInnerSizeY1),
                    scope_rect.Max);
        return scope_rect;
    }
}
                                            ...
                                            int items_count)
{
    ...
    if (ImGuiTable* table = g.CurrentTable)
    {
        ...
        {
            TableEndCell(table); // This is currently safe to call multiple time. If that properly
                                 // is lost we can extract the "save measurement" part of it.
        }
    }
        ...
        && g.NavJustMovedToHasSelectionData)
    {
        ...
        if ((ms->KeyMods & (ImGuiMod_Ctrl | ImGuiMod_Shift)) == 0
            && (flags & (ImGuiMultiSelectFlags_NoAutoClear | ImGuiMultiSelectFlags_NoAutoSelect))
                   == 0)
        {
            request_clear = true;
        }
    ...
    if (ms->IsFocused)
    {
        ...
        if (ms->IO.RangeSrcReset
            || (ms->RangeSrcPassedBy == false
                && ms->IO.RangeSrcItem
                       != ImGuiSelectionUserData_Invalid)) // Can't read storage->RangeSrcItem here
                                                           // -> we want the state at beginning of
        ...
        if (ms->IsKeyboardSetRange)
        {
            ...
            if (is_range_dst
                && storage->RangeSrcItem
                       == ImGuiSelectionUserData_Invalid) // If we don't have RangeSrc, assign
                                                          // RangeSrc = RangeDst
            {
...
void ImGui::MultiSelectItemFooter(ImGuiID id, bool* p_selected, bool* p_pressed)
{
    ...
    if (ms->BoxSelectId != 0)
    {
        if (ImGuiBoxSelectState* bs = GetBoxSelectState(ms->BoxSelectId))
        {
                ...
                || (rect_overlap_prev && !rect_overlap_curr))
            {
                ...
                {
                    pressed = true; // First item act as a pressed: code below will emit selection
                                    // request and set NavId (whatever we emit here will be
                                    // overridden anyway)
    ...
    if (pressed && (!enter_pressed || !selected))
    {
        ...
        MultiSelectAddSetRange(ms, range_selected, range_direction, storage->RangeSrcItem,
                               item_data);
    }
                                   ...
                                   ImGuiSelectionUserData last_item)
{
    ...
    ImGuiSelectionRequest req
        = { ImGuiSelectionRequestType_SetRange, selected, (ImS8)range_dir,
...
{
    #ifndef IMGUI_DISABLE_DEBUG_TOOLS
    const bool is_active = (storage->LastFrameActive
...
bool ImGui::BeginListBox(const char* label, glm::vec2 size_arg)
{
    ...
    if (label_size.x > 0.0f)
    {
        glm::vec2 label_pos = glm::vec2(frame_bb.Max.x + style.ItemInnerSpacing.x,
                                        frame_bb.Min.y + style.FramePadding.y);
        RenderText(label_pos, label);
                    ...
                    int items_count, int height_in_items)
{
    ...
    glm::vec2 size(0.0f, ImTrunc(GetTextLineHeightWithSpacing() * height_in_items_f
                  ...
                  glm::vec2 size_arg)
{
    ...
    const ImRect total_bb(
        frame_bb.Min,
        frame_bb.Max
            + glm::vec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    if (values_count >= values_count_min)
    {
        ...
        for (int n = 0; n < res_w; n++)
        {
            ...
            glm::vec2 pos1 = ImLerp(
                inner_bb.Min, inner_bb.Max,
                (plot_type == ImGuiPlotType_Lines) ? tp1 : glm::vec2(tp1.x, histogram_zero_line_t));
            if (plot_type == ImGuiPlotType_Lines)
            {
    ...
    {
        RenderTextClipped(glm::vec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y),
                          frame_bb.Max, overlay_text, NULL, NULL, glm::vec2(0.5f, 0.0f));
    }
...
void ImGui::PlotLines(const char* label, const float* values, int values_count, int values_offset,
                      ...
                      glm::vec2 graph_size, int stride)
{
...
void ImGui::PlotLines(const char* label, float (*values_getter)(void* data, int idx), void* data,
                      ...
                      float scale_min, float scale_max, glm::vec2 graph_size)
{
...
void ImGui::PlotHistogram(const char* label, const float* values, int values_count,
                          ...
                          float scale_max, glm::vec2 graph_size, int stride)
{
    ImGuiPlotArrayGetterData data(values, stride);
...
bool ImGui::BeginMenuBar()
{
    ...
    const float border_top
        = ImMax(IM_ROUND(window->WindowBorderSize * 0.5f - window->TitleBarHeight), 0.0f);
    const float border_half = IM_ROUND(window->WindowBorderSize * 0.5f);
    ImRect bar_rect = window->MenuBarRect();
    ImRect clip_rect(
        ImFloor(bar_rect.Min.x + border_half), ImFloor(bar_rect.Min.y + border_top),
    ...
    window->DC.CursorPos = window->DC.CursorMaxPos = glm::vec2(
...
void ImGui::EndMenuBar()
{
        ...
        && (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
    {
            ...
            && (g.NavMoveFlags & ImGuiNavMoveFlags_Forwarded) == 0)
        {
            ...
            {
                g.NavCursorVisible = false; // Hide nav cursor for the current frame so we don't see
                                            // the intermediary selection. Will be set again
...
bool ImGui::BeginMainMenuBar()
{
    ...
    g.NextWindowData.MenuBarOffsetMinVal
        = glm::vec2(g.Style.DisplaySafeAreaPadding.x,
    ...
    // Temporarily disable _NoSavedSettings, in the off-chance that tables or child windows
    // submitted within the menu-bar may want to use settings. (#8356)
...
void ImGui::EndMainMenuBar()
{
    ...
    if (!g.CurrentWindow->DC.MenuBarAppending)
    {
        IM_ASSERT_USER_ERROR(
            0, "Calling EndMainMenuBar() not from a menu-bar!"); // Not technically testing that it
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (g.MenusIdSubmittedThisFrame.contains(id))
    {
        if (menu_is_open)
        {
            menu_is_open = BeginPopupMenuEx(
                id, label, window_flags); // menu_is_open can be 'false' when the popup is
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        popup_pos = glm::vec2(pos.x - 1.0f - IM_TRUNC(style.ItemSpacing.x * 0.5f),
        ...
        glm::vec2 text_pos(window->DC.CursorPos.x + offsets->OffsetLabel,
                           window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
    ...
    else
    {
        ...
        RenderArrow(window->DrawList,
                    pos + glm::vec2(offsets->OffsetMark + extra_w + g.FontSize * 0.30f, 0.0f),
    ...
    if (menu_is_open)
    {
        ...
        PushStyleVar(ImGuiStyleVar_ChildRounding,
                     style.PopupRounding); // First level will use _PopupRounding, subsequent will
                                           // use _ChildRounding
        menu_is_open = BeginPopupMenuEx(
            id, label, window_flags); // menu_is_open may be 'false' when the popup is completely
    ...
    else
    {
        ...
        if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible)
        {
            ...
            if (shortcut_w > 0.0f)
            {
                ...
                LogSetNextTextDecoration("(", ")");
                RenderText(pos + glm::vec2(offsets->OffsetShortcut + stretch_w, 0.0f), shortcut,
                           NULL, false);
                PopStyleColor();
            }
            if (selected)
            {
                RenderCheckMark(
                    window->DrawList,
                    pos
                        + glm::vec2(offsets->OffsetMark + stretch_w + g.FontSize * 0.40f,
...
bool ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    {
        window->DC.CursorPos
            = glm::vec2(tab_bar->BarRect.Min.x, tab_bar->BarRect.Max.y + tab_bar->ItemSpacingY);
        tab_bar->BeginCount++;
        return true;
    }
    ...
    if (g.Style.TabBarBorderSize > 0.0f)
    {
        ...
        window->DrawList->AddRectFilled(
            glm::vec2(tab_bar->SeparatorMinX, y - g.Style.TabBarBorderSize),
            glm::vec2(tab_bar->SeparatorMaxX, y), col);
    }
    return true;
}
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    tab_bar->ScrollButtonEnabled
        = ((width_all_tabs_after_min_width_shrink > tab_bar->BarRect.GetWidth()
            && tab_bar->Tabs.Size > 1)
    ...
    ItemSize(glm::vec2(tab_bar->WidthAllTabs, tab_bar->BarRect.GetHeight()),
...
void ImGui::TabBarAddTab(ImGuiTabBar* tab_bar, ImGuiTabItemFlags tab_flags, ImGuiWindow* window)
{
    ...
    IM_ASSERT(g.CurrentTabBar
              != tab_bar); // Can't work while the tab bar is active as our tab doesn't have an X
                           ...
                           // triggers the commented early-out assert in BeginTabBarEx() (probably
                                           ...
                                           glm::vec2 mouse_pos)
{
    ...
    for (int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir)
    {
        ...
        // GetForegroundDrawList()->AddRect(glm::vec2(x1, tab_bar->BarRect.Min.y), glm::vec2(x2,
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    const glm::vec2 arrow_button_size(g.FontSize - 2.0f,
                                      g.FontSize + g.Style.FramePadding.y * 2.0f);
    const float scrolling_buttons_width = arrow_button_size.x * 2.0f;

    const glm::vec2 backup_cursor_pos = window->DC.CursorPos;
    // window->DrawList->AddRect(glm::vec2(tab_bar->BarRect.Max.x - scrolling_buttons_width,
    // tab_bar->BarRect.Min.y), glm::vec2(tab_bar->BarRect.Max.x, tab_bar->BarRect.Max.y),
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    const glm::vec2 backup_cursor_pos = window->DC.CursorPos;
    window->DC.CursorPos
        = glm::vec2(tab_bar->BarRect.Min.x - g.Style.FramePadding.y, tab_bar->BarRect.Min.y);
                      ...
                      ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    {
        window->DC.CursorPos = tab_bar->BarRect.Min
                               + glm::vec2(IM_TRUNC(tab->Offset - tab_bar->ScrollingAnim), 0.0f);
    }
    else
    {
    ...
    if (want_clip_rect)
    {
        PushClipRect(glm::vec2(ImMax(bb.Min.x, tab_bar->ScrollingRectMinX), bb.Min.y - 1),
                     glm::vec2(tab_bar->ScrollingRectMaxX, bb.Max.y), true);
    }
    ...
    if (g.DragDropActive
        && !g.DragDropPayload.IsDataType(
            IMGUI_PAYLOAD_TYPE_WINDOW)) // FIXME: May be an opt-in property of the payload to
                                        // disable this
    {
        button_flags |= ImGuiButtonFlags_PressedOnDragDropHold;
    }
    ...
    else if (held && !tab_appearing && IsMouseDragging(0))
    {
        ...
        if (can_undock)
        {
            ...
            if (!undocking_tab) //&& (!g.IO.ConfigDockingWithShift || g.IO.KeyShift)
            {
                ...
                // GetForegroundDrawList()->AddRect(glm::vec2(bb.Min.x - threshold_x, bb.Min.y -
                // threshold_y), glm::vec2(bb.Max.x + threshold_x, bb.Max.y + threshold_y),
    ...
    if (is_visible)
    {
            ...
            && style.TabBarOverlineSize > 0.0f)
        {
            ...
            if (style.TabRounding > 0.0f)
            {
                ...
                display_draw_list->PathArcToFast(tl + glm::vec2(+rounding, +rounding), rounding, 7,
                                                 9);
                display_draw_list->PathArcToFast(tr + glm::vec2(-rounding, +rounding), rounding, 9,
                                                 11);
                display_draw_list->PathStroke(overline_col, 0, style.TabBarOverlineSize);
            }
            else
            {
                display_draw_list->AddLine(tl - glm::vec2(0.5f, 0.5f), tr - glm::vec2(0.5f, 0.5f),
        ...
        // technically hovering our parent) That state is copied to window->DockTabItemStatusFlags
        // by our caller.
        ...
        // HoveredIdTimer-> seems ok) (We test IsItemHovered() to discard e.g. when another item is
        // active or drag and drop over the tab bar, which g.HoveredId ignores)
...
glm::vec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button_or_unsaved_marker)
{
    ImGuiContext& g = *GImGui;
    glm::vec2 label_size = CalcTextSize(label, NULL, true);
    glm::vec2 size = glm::vec2(label_size.x + g.Style.FramePadding.x,
                              ...
                              ImU32 col)
{
    ...
    if (g.Style.TabBorderSize > 0.0f)
    {
        ...
        draw_list->PathArcToFast(glm::vec2(bb.Min.x + rounding + 0.5f, y1 + rounding + 0.5f),
                                 rounding, 6, 9);
        draw_list->PathArcToFast(glm::vec2(bb.Max.x - rounding - 0.5f, y1 + rounding + 0.5f),
                                 rounding, 9, 12);
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb,
                                       ImGuiTabItemFlags flags, glm::vec2 frame_padding,
                                       ...
                                       bool* out_text_clipped)
{
    ...
    {
        *out_text_clipped
            = (text_ellipsis_clip_bb.Min.x + label_size.x) > text_ellipsis_clip_bb.Max.x;
        // draw_list->AddCircle(text_ellipsis_clip_bb.Min, 3.0f, *out_text_clipped ? IM_COL32(255,
        // 0, 0, 255) : IM_COL32(0, 255, 0, 255));
    }

    const float button_sz = g.FontSize;
    const glm::vec2 button_pos(ImMax(bb.Min.x, bb.Max.x - frame_padding.x - button_sz),
        ...
        else
        {
            close_button_visible
                = (g.Style.TabCloseButtonMinWidthUnselected < 0.0f)
                      ...
                      : (is_hovered
                         && bb.GetWidth()
                                >= ImMax(button_sz, g.Style.TabCloseButtonMinWidthUnselected));
        }
    }
