...
struct ImGuiWindowDockStyle; // Storage for window-style data which needs to be stored for docking
                             // purpose
...
extern IMGUI_API ImGuiContext* GImGui; // Current implicit context pointer
    ...
    // otherwise would slow down the code too much. We currently don't have many of those so the
    // effect is currently negligible, but onward intent to add more aggressive ones in the code.
    ...
    // In a real application you should be using PRId64 and PRIu64 from <inttypes.h> (non-windows)
    // and on Windows define them yourself.
    #if defined(_MSC_VER) && !defined(__clang__)
...
// Helpers: Maths
IM_MSVC_RUNTIME_CHECKS_OFF
    // - Wrapper for standard libs functions. (Note that imgui_demo.cpp does _not_ use them to keep
...
inline glm::vec4 ImLerp(const glm::vec4& a, const glm::vec4& b, float t)
{
    return glm::vec4(a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t, a.z + (b.z - a.z) * t,
                     a.w + (b.w - a.w) * t);
}
...
inline float ImLengthSqr(const glm::vec4& lhs)
{
...
struct IMGUI_API ImRect
{
    ...
    constexpr ImRect(const glm::vec4& v)
        : Min(v.x, v.y),
          Max(v.z, v.w)
    {
    }
    ...
    glm::vec2 GetTL() const { return Min; }                     // Top-left
...
IMGUI_API ImGuiStoragePair* ImLowerBound(ImGuiStoragePair* in_begin, ImGuiStoragePair* in_end,
    ...
    // Our equation is significantly simpler that one in the post thanks for choosing segment that
    // is perpendicular to X axis. Follow steps in the article from this starting condition and you
    // will will get this result.
    //
...
struct IMGUI_API ImDrawListSharedData
{
    ...
    glm::vec4 ClipRectFullscreen; // Value for PushClipRectFullscreen()
    ImVector<glm::vec2> TempBuffer;  // Temporary write buffer
    ImVector<ImDrawList*> DrawLists; // All draw lists associated to this ImDrawListSharedData
    ImGuiContext* Context; // [OPTIONAL] Link to Dear ImGui context. 99% of ImDrawList/ImFontAtlas
                           // can function without an ImGui context, but this facilitate handling
                           // one legacy edge case.
...
struct ImGuiStyleMod
{
    ...
    ImGuiStyleMod(ImGuiStyleVar idx, glm::vec2 v)
    {
        VarIdx = idx;
        BackupFloat[0] = v.x;
        BackupFloat[1] = v.y;
...
enum ImGuiItemFlagsPrivate_
{
    ...
    ImGuiItemFlags_Disabled
        = 1 << 10, // false     // Disable interactions (DOES NOT affect visuals. DO NOT mix direct
                   // use of this with BeginDisabled(). See BeginDisabled()/EndDisabled() for full
                   // disable feature, and github #211).
    ...
    ImGuiItemFlags_NoNavDisableMouseHover
        = 1 << 15, // false     // Nav keyboard/gamepad mode doesn't disable hover highlight (behave
                   // as if NavHighlightItemUnderNav==false).
    ImGuiItemFlags_NoMarkEdited = 1 << 16, // false     // Skip calling MarkItemEdited()
    ImGuiItemFlags_NoFocus
        = 1 << 17, // false     // [EXPERIMENTAL: Not very well specced] Clicking doesn't take
                   // focus. Automatically sets ImGuiButtonFlags_NoFocus +
                   // ImGuiButtonFlags_NoNavFocus in ButtonBehavior().

    // Controlled by widget code
    ImGuiItemFlags_Inputable
        = 1 << 20, // false     // [WIP] Auto-activate input mode when tab focused. Currently only
...
enum ImGuiItemStatusFlags_
{
    ...
    ImGuiItemStatusFlags_Visible = 1 << 8, // [WIP] Set when item is overlapping the current
    ...
    // ImGuiItemStatusFlags_FocusedByTabbing = 1 << 8,   // Removed IN 1.90.1 (Dec 2023). The
    // trigger is part of g.NavActivateId. See commit 54c1bdeceb.
...
enum ImGuiInputTextFlagsPrivate_
{
    ...
    ImGuiInputTextFlags_MergedItem
        = 1 << 27, // For internal use by TempInputText(), will skip calling ItemAdd(). Require
...
enum ImGuiButtonFlagsPrivate_
{
    ...
    ImGuiButtonFlags_AllowOverlap = 1 << 12, // require previous frame HoveredId to either match id
                                             // or be null before being usable.
    // ImGuiButtonFlags_DontClosePopups      = 1 << 13,  // disable automatically closing parent
    // popup on press ImGuiButtonFlags_Disabled             = 1 << 14,  // disable interactions ->
    ...
    ImGuiButtonFlags_NoNavFocus
        = 1 << 18, // don't override navigation focus when activated (FIXME: this is essentially
                   // used every time an item uses ImGuiItemFlags_NoNav, but because legacy specs
    ...
    // ImGuiButtonFlags_NoKeyModifiers       = ImGuiButtonFlags_NoKeyModsAllowed, // Renamed
    // in 1.91.4
};
...
enum ImGuiSelectableFlagsPrivate_
{
    ...
    ImGuiSelectableFlags_SelectOnNav
        = 1 << 21, // (WIP) Auto-select when moved into. This is not exposed in public API as to
                   // handle multi-select and modifiers we will need user to explicitly control
                   // focus scope. May be replaced with a BeginSelection() API.
    ...
    ImGuiSelectableFlags_SpanAvailWidth
        = 1 << 24, // Span all avail width even if we declared less for layout purpose. FIXME: We
                   // may be able to remove this (added in 6251d379, 2bcafc86 for menus)
...
// Horizontal/Vertical enums are fixed to 0/1 so they may be used to index glm::vec2
...
// X/Y enums are fixed to 0/1 so they may be used to index glm::vec2
enum ImGuiAxis
{
    ImGuiAxis_None = -1,
    ImGuiAxis_X = 0,
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    const char*
        TextSrc; // == TextA.Data unless read-only, in which case == buf passed to InputText().
                 // Field only set and valid _inside_ the call InputText() call.
    ...
    // If you modify underlying user-passed const char* while active you need to call this
    // (InputText V2 may lift this)
    //   strcpy(my_buf, "hello");
...
// extension? Using glm::vec2() would fail on Clang 'union member 'MousePos' has a non-trivial
// default constructor'
...
enum ImGuiInputFlagsPrivate_
{
    ...
    ImGuiInputFlags_RepeatUntilKeyModsChange = 1
                                               << 5, // Stop repeating when released OR if keyboard
                                                     // mods are changed (default for Shortcut)
    ...
    // - Locking key away from non-input aware code. Locking is useful to make input-owner-aware
    // code steal keys from non-input-owner-aware code. If all code is input-owner-aware locking
    // would never be necessary.
    ImGuiInputFlags_LockThisFrame
        = 1 << 20, // Further accesses to key data will require EXPLICIT owner ID
                   // (ImGuiKeyOwner_Any/0 will NOT accepted for polling). Cleared at end of frame.
    ImGuiInputFlags_LockUntilRelease
        = 1 << 21, // Further accesses to key data will require EXPLICIT owner ID
                   // (ImGuiKeyOwner_Any/0 will NOT accepted for polling). Cleared when the key is
                   // released or at end of each frame if key is released.
...
enum ImGuiNavRenderCursorFlags_
{
    ...
    ImGuiNavHighlightFlags_NoRounding
        = ImGuiNavRenderCursorFlags_NoRounding, // Renamed in 1.91.4
                                                ...
                                                // ImGuiNavRenderCursorFlags_Compact,    // Renamed
                                                // in 1.90.2
    #endif
};
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_FocusApi
        = 1 << 9, // Requests from focus API can land/focus/activate items even if they are marked
...
enum ImGuiTypingSelectFlags_
{
    ...
    ImGuiTypingSelectFlags_AllowBackspace
        = 1 << 0, // Backspace to delete character inputs. If using: ensure GetTypingSelectRequest()
                  // is not called more than once per frame (filter by e.g. focus state)
...
struct IMGUI_API ImGuiTypingSelectRequest
{
    ...
    bool SingleCharMode;      // Notify when buffer contains same character repeated, to implement
                              // special mode. In this situation it preferred to not display any
...
struct IMGUI_API ImGuiTypingSelectState
{
    ...
    char SearchBuffer[64]; // Search buffer: no need to make dynamic as this search is very
                           // transient.
    ImGuiID FocusScope;
    int LastRequestFrame = 0;
    float LastRequestTime = 0.0f;
    bool SingleCharModeLock
        = false; // After a certain single char repeat count we lock into SingleCharMode. Two
                 // benefits: 1) buffer never fill, 2) we can provide an immediate SingleChar mode
                 // without timer elapsing.
...
struct IMGUI_API ImGuiMultiSelectTempData
{
    ...
    bool IsFocused; // Set if currently focusing the selection scope (any item of the selection).
...
enum ImGuiDockNodeFlagsPrivate_
{
    ...
    ImGuiDockNodeFlags_DockSpace
        = 1 << 10, // Saved // A dockspace is a node that occupy space within an existing user
                   // window. Otherwise the node is floating and create its own window.
    ImGuiDockNodeFlags_CentralNode
        = 1 << 11, // Saved // The central node has 2 main properties: stay visible when empty, only
                   // use "remaining" spaces from its neighbor.
    ImGuiDockNodeFlags_NoTabBar = 1 << 12, // Saved // Tab bar is completely unavailable. No
                                           // triangle in the corner to enable it back.
    ImGuiDockNodeFlags_HiddenTabBar
        = 1 << 13, // Saved // Tab bar is hidden, with a triangle in the corner to show it again
                   // (NB: actual tab-bar instance may be destroyed as this is only used for
                   // single-window tab bar)
    ImGuiDockNodeFlags_NoWindowMenuButton = 1
                                            << 14, // Saved // Disable window/docking menu (that one
                                                   // that appears instead of the collapse button)
    ...
    ImGuiDockNodeFlags_DockedWindowsInFocusRoute
        = 1 << 18, //       // Any docked window will be automatically be focus-route chained
                   //       (window->ParentWindowForFocusRoute set to this) so Shortcut() in this
                   //       window can run when any docked window is focused.

    // Disable docking/undocking actions in this dockspace or individual node (existing docked nodes
    // will be preserved) Those are not exposed in public because the desirable
    // sharing/inheriting/copy-flag-on-split behaviors are quite difficult to design and understand.
    // The two public flags
    // ImGuiDockNodeFlags_NoDockingOverCentralNode/ImGuiDockNodeFlags_NoDockingSplit don't have
    // those issues.
    ...
    ImGuiDockNodeFlags_NoDockingOverOther = 1 << 21, //       // Disable this node from being docked
                                                     //       over another window or non-empty node.
    ImGuiDockNodeFlags_NoDockingOverEmpty
        = 1 << 22, //       // Disable this node from being docked over an empty node (e.g.
...
// We don't store style.Alpha: dock_node->LastBgColor embeds it and otherwise it would only affect
// the docking tab, which intuitively I would say we don't want to.
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    int BgFgDrawListsLastFrame[2]; // Last frame number the background (0) and foreground (1) draw
                                   // lists were used
    ImDrawList* BgFgDrawLists[2];  // Convenience background (0) and foreground (1) draw lists. We
    ...
    ImGuiViewportP()
    {
        ...
        LastPlatformPos = LastPlatformSize = LastRendererSize = glm::vec2(FLT_MAX, FLT_MAX);
    }
    ~ImGuiViewportP()
    {
        if (BgFgDrawLists[0])
    ...
    {
        glm::vec2 pos = CalcWorkRectPos(BuildWorkInsetMin);
        glm::vec2 size = CalcWorkRectSize(BuildWorkInsetMin, BuildWorkInsetMax);
...
{
    ImGuiID ID;
    glm::i16vec2 Pos; // NB: Settings position are stored RELATIVE to the viewport! Whereas runtime
                      // ones are absolute positions.
    glm::i16vec2 Size;
    glm::i16vec2 ViewportPos;
    ImGuiID ViewportId;
...
struct ImGuiLocEntry
{
    ...
    // - The message will always be a string literal, in order to increase likelihood of being
    // display by an assert handler.
...
struct ImGuiContext
{
    ...
    float FontBakedScale; // == FontBaked->Size / FontSize. Scale factor over baked size. Rarely
                          // used nowadays, very often == 1.0f.
    ...
    char ContextName[16];     // Storage for a context name (to facilitate debugging multi-context
                              // setups)
    ...
    float
        WindowsBorderHoverPadding; // Padding around resizable windows for which hovering on counts
    ...
    ImGuiWindow*
        HoveredWindowBeforeClear; // Window the mouse is hovering. Filled even with _NoMouse. This
                                  // is currently useful for multi-context compositors.
    ImGuiWindow*
    ...
    int PlatformWindowsCreatedCount; // Unique sequential creation counter (mostly for
                                     // testing/debugging)
    ...
    // before 1.91.4 (2024/10/18). OPPOSITE VALUE (g.NavDisableHighlight == !g.NavCursorVisible)
    // bool                  NavDisableMouseHover;               // Old name for
    // g.NavHighlightItemUnderNav before 1.91.1 (2024/10/18) this was called When user starts using
    ...
    bool NavJustMovedToIsTabbing; // Copy of ImGuiNavMoveFlags_IsTabbing. Maybe we should store
                                  // whole flags.
    ...
    bool ConfigNavWindowingWithGamepad; // = true. Enable CTRL+TAB by holding
                                        // ImGuiKey_GamepadFaceLeft (== ImGuiKey_NavGamepadMenu).
                                        // When false, the button may still be used to toggle Menu
                                        // layer.
    ImGuiKeyChord
    ...
    ImGuiID HoverItemUnlockedStationaryId;   // Mouse has once been stationary on this item. Only
                                             // reset after departing the item.
    ...
    glm::vec4 ColorPickerRef;  // Initial/reference color at the time of opening the color picker.
    ...
    // Platform support
    ImGuiPlatformImeData PlatformImeData; // Data updated by current frame. Will be applied at end
                                          // of the frame. For some backends, this is required to
                                          // have WantVisible=true in order to receive text message.
    ...
    glm::vec4 DebugFlashStyleColorBackup;
...
struct IMGUI_API ImGuiWindow
{
    ...
    float TitleBarHeight,
        MenuBarHeight; // Note that those used to be function before 2024/05/28. If you have old
                       // code calling TitleBarHeight() you can change it to TitleBarHeight.
    ...
    glm::i16vec2 HitTestHoleSize; // Define an optional rectangular hole where mouse will
                                  // pass-through the window.
    glm::i16vec2 HitTestHoleOffset;
...
public:
    ...
    {
        return ImRect(Pos, glm::vec2(Pos.x + SizeFull.x, Pos.y + TitleBarHeight));
    }
    ...
    // FontWindowScale * FontDpiScale * FontWindowScaleParents;
};
...
struct IMGUI_API ImGuiTabBar
{
    ...
    ImRect BarRect;
    float BarRectPrevWidth; // Backup of previous width. When width change we enforce keep
                            // horizontal scroll on focused tab.
...
struct ImGuiTableInstanceData
{
    ...
    float LastTopHeadersRowHeight; // Height of first consecutive header rows from last frame
                                   // (FIXME: this is used assuming consecutive headers are in same
                                   // frozen set)
...
struct IMGUI_API ImGuiTable
{
    ...
    ImS16 InstanceCurrent; // Count of BeginTable() calls with same ID in the same frame (generally
                           ...
                           // multiple tables with the same ID are multiple tables, they are just
                           // synced.
...
namespace ImGui
{
    ...
    IMGUI_API glm::vec2 CalcWindowNextAutoFitSize(ImGuiWindow* window);
    ...
    IMGUI_API glm::vec2 ScrollToRectEx(ImGuiWindow* window, const ImRect& rect,
                                       ImGuiScrollFlags flags = 0);
    // #ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    ...
    IMGUI_API void LogToBuffer(int auto_open_depth
                               = -1); // Start logging/capturing to internal buffer
    IMGUI_API void LogRenderedText(const glm::vec2* ref_pos, const char* text,
    ...
    // implement than expected. I have a couple of private branches on this matter but it's not
    // simple. For now implementing the easy ones.
    IMGUI_API void FocusItem(); // Focus last item (no selection/activation).
    IMGUI_API void ActivateItemByID(
        ImGuiID id); // Activate an item by ID (button, checkbox, tree node etc.). Activation is
                     // queued and processed on the next frame when the item is encountered again.
                     // Was called 'ActivateItem()' before 1.89.7.
    ...
    IMGUI_API glm::vec2 GetKeyMagnitude2d(ImGuiKey key_left, ImGuiKey key_right, ImGuiKey key_up,
                                          ImGuiKey key_down);
    ...
    IMGUI_API bool IsKeyPressed(
        ...
        ImGuiID owner_id = 0); // Important: when transitioning from old to new IsKeyPressed(): old
    ...
    //   - The default route is ImGuiInputFlags_RouteFocused (accept inputs if window is in focus
    //   stack. Deep-most focused window takes inputs. ActiveId takes inputs over deep-most focused
    //   window.)
    ...
    //   - Each route may be granted to a single owner. When multiple requests are made we have
    //   policies to select the winning route (e.g. deep most window).
    //   - Multiple read sites may use the same owner id can all access the granted route.
    //   - When owner_id is 0 we use the current Focus Scope ID as a owner ID in order to identify
    //   our location.
    // - You can chain two unrelated windows in the focus stack using
    // SetWindowParentWindowForFocusRoute()
    //   e.g. if you have a tool window associated to a document, and you want document shortcuts to
    //   run when the tool is focused.
    ...
    IMGUI_API bool DockContextCalcDropPosForDocking(ImGuiWindow* target, ImGuiDockNode* target_node,
                                                    ...
                                                    bool split_outer, glm::vec2* out_pos);
    ...
    IMGUI_API void DockBuilderSetNodePos(ImGuiID node_id, glm::vec2 pos);
    IMGUI_API void DockBuilderSetNodeSize(ImGuiID node_id, glm::vec2 size);
    ...
    // Typing-Select API
    // (provide Windows Explorer style "select items by typing partial name" + "cycle through items
    // by typing same letter" feature) (this is currently not documented nor used by main library,
    // but should work. See "widgets_typingselect" in imgui_test_suite for usage code. Please let us
    // know if you use this!)
    IMGUI_API ImGuiTypingSelectRequest* GetTypingSelectRequest(ImGuiTypingSelectFlags flags
    ...
    IMGUI_API bool BeginBoxSelect(const ImRect& scope_rect, ImGuiWindow* window,
                                  ImGuiID box_select_id, ImGuiMultiSelectFlags ms_flags);
    ...
    IMGUI_API int TableGetHoveredRow(); // Retrieve *PREVIOUS FRAME* hovered row. This difference
    ...
    IMGUI_API glm::vec2 TabItemCalcSize(const char* label, bool has_close_button_or_unsaved_marker);
    IMGUI_API glm::vec2 TabItemCalcSize(ImGuiWindow* window);
    ...
    // internally)
    IMGUI_API void RenderText(glm::vec2 pos, const char* text, const char* text_end = NULL,
    ...
    IMGUI_API void RenderFrameBorder(glm::vec2 p_min, glm::vec2 p_max, float rounding = 0.0f);
    IMGUI_API void RenderColorRectWithAlphaCheckerboard(ImDrawList* draw_list, glm::vec2 p_min,
                                                        glm::vec2 p_max, ImU32 fill_col,
                                                        float grid_step, glm::vec2 grid_off,
                                                        float rounding = 0.0f,
                                                        ImDrawFlags flags = 0);
    IMGUI_API void RenderNavCursor(const ImRect& bb, ImGuiID id,
    ...
    IMGUI_API void RenderMouseCursor(glm::vec2 pos, float scale, ImGuiMouseCursor mouse_cursor,
                                     ImU32 col_fill, ImU32 col_border, ImU32 col_shadow);

    // Render helpers (those functions don't access any ImGui state!)
    IMGUI_API void RenderArrow(ImDrawList* draw_list, glm::vec2 pos, ImU32 col, ImGuiDir dir,
                               float scale = 1.0f);
    IMGUI_API void RenderBullet(ImDrawList* draw_list, glm::vec2 pos, ImU32 col);
    IMGUI_API void RenderCheckMark(ImDrawList* draw_list, glm::vec2 pos, ImU32 col, float sz);
    IMGUI_API void RenderArrowPointingAt(ImDrawList* draw_list, glm::vec2 pos, glm::vec2 half_sz,
                                         ImGuiDir direction, ImU32 col);
    IMGUI_API void RenderArrowDockMenu(ImDrawList* draw_list, glm::vec2 p_min, float sz, ImU32 col);
    ...
    } // This may be useful to apply workaround that a based on distinguish whenever an item is
      // active as a text input field.
    ...
    IMGUI_API void ShadeVertsLinearColorGradientKeepAlpha(ImDrawList* draw_list, int vert_start_idx,
                                                          int vert_end_idx, glm::vec2 gradient_p0,
    ...
    IMGUI_API void DebugNodePlatformMonitor(ImGuiPlatformMonitor* monitor, const char* label,
                                            int idx);
    ...
    // Obsolete functions
        ...
        // IM_ASSERT(IsNamedKey(key)); return IsKeyPressed(key, repeat); } // Removed in 1.87:
        // Mapping from named key is always identity!
        ...
        //  'ItemAdd(..., ImGuiItemAddFlags_Focusable)'  and 'bool tab_focused =
        //  (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Focused) != 0' (New)
        //  IMGUI_VERSION_NUM >= 18413: using 'ItemAdd(..., ImGuiItemFlags_Inputable)'     and 'bool
        //  tab_focused = (g.NavActivateId == id && (g.NavActivateFlags &
        ...
        // FocusableItemUnregister(ImGuiWindow* window)                        // -> unnecessary:
...
inline bool operator!=(const ImTextureRef& lhs, const ImTextureRef& rhs)
{
    ...
    #define ImFontAtlasRectId_IndexMask_                                                           \
        (0x000FFFFF) // 20-bits: index to access builder->RectsIndex[].
    #define ImFontAtlasRectId_GenerationMask_                                                      \
        (0x3FF00000) // 10-bits: entry generation, so each ID is unique and get can safely detected
                     // old identifiers.
...
inline ImFontAtlasRectId ImFontAtlasRectId_Make(int index_idx, int gen_idx)
{
    IM_ASSERT(index_idx < ImFontAtlasRectId_IndexMask_
              && gen_idx
                     < (ImFontAtlasRectId_GenerationMask_ >> ImFontAtlasRectId_GenerationShift_));
...
{
    int TargetIndex : 20; // When Used: ImFontAtlasRectId -> into Rects[]. When unused: index to
                          // next unused RectsIndex[] slot to consume free-list.
...
{
    stbrp_context_opaque PackContext; // Actually 'stbrp_context' but we don't want to define this
                                      // in the header file.
    ...
    glm::ivec2 MaxRectSize; // Largest rectangle to pack (de-facto used as a "minimum texture size")
    glm::ivec2 MaxRectBounds;      // Bottom-right most used pixels
    bool LockDisableResize;        // Disable resizing texture
    bool PreloadedAllGlyphsRanges; // Set when missing ImGuiBackendFlags_RendererHasTextures
...
IMGUI_API glm::ivec2 ImFontAtlasTextureGetSizeEstimate(ImFontAtlas* atlas);
...
IMGUI_API bool ImFontAtlasGetMouseCursorTexData(ImFontAtlas* atlas, ImGuiMouseCursor cursor_type,
                                                glm::vec2* out_offset, glm::vec2* out_size,
                                                glm::vec2 out_uv_border[2],
                                                glm::vec2 out_uv_fill[2]);
