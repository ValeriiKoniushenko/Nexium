...
struct R<Core::Scene>
{
    ...
    static constexpr std::string_view FullName() { return "Core::Scene"; }
    ...
    static void Deserialize(const RResourceStream<RImpl>& s, Core::Scene& obj, bool noSignals = false)
    {
		...
		s.read("_sceneName", obj._sceneName, "Default", 16);
        if (!noSignals)
        {
    ...
    static bool GetField(Core::Scene& obj, const char* fieldName, const std::function<void(void*, const char*)>& onFound)
    {
        if (!onFound) return false;
        if (std::strcmp(fieldName, "_sceneName") == 0)
        {
            onFound((&obj._sceneName), "StringAtom");
            return true;
        }
