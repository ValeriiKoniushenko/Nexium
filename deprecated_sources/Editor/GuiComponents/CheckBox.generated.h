...
struct R<Core::Gui::CheckBox>
{
    ...
    static constexpr std::string_view FullName() { return "Core::Gui::CheckBox"; }
    ...
    static void Deserialize(const RResourceStream<RImpl>& s, Core::Gui::CheckBox& obj, bool noSignals = false)
    {
		...
		s.read("_currentValue", obj._currentValue, false, 16);
        if (!noSignals)
        {
    ...
    static bool GetField(Core::Gui::CheckBox& obj, const char* fieldName, const std::function<void(void*, const char*)>& onFound)
    {
        ...
        if (std::strcmp(fieldName, "_currentValue") == 0)
        {
            onFound((&obj._currentValue), "bool");
            return true;
        }
