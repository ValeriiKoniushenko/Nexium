...
struct R<Core::Gui::BaseList>
{
    ...
    static constexpr std::string_view FullName() { return "Core::Gui::BaseList"; }
    ...
    static void Serialize(const Core::Gui::BaseList& obj, RResourceStream<RImpl>& s, bool noSignals = false)
    {
		...
		s.write("_currentIndex", obj._currentIndex);
        if (!noSignals)
        {
    ...
    static void Deserialize(const RResourceStream<RImpl>& s, Core::Gui::BaseList& obj, bool noSignals = false)
    {
		...
		s.read("_filter", obj._filter, decltype(obj._filter){}, 16);
		s.read("_size", obj._size, glm::vec2(100.f, 150.f), 16);
		s.read("_currentIndex", obj._currentIndex, 0, 16);
        if (!noSignals)
        {
    ...
    static bool GetField(Core::Gui::BaseList& obj, const char* fieldName, const std::function<void(void*, const char*)>& onFound)
    {
        if (!onFound) return false;
        if (std::strcmp(fieldName, "_filter") == 0)
        {
            onFound((&obj._filter), "Core::StringAtom");
            return true;
        }
        ...
        if (std::strcmp(fieldName, "_currentIndex") == 0)
        {
            onFound((&obj._currentIndex), "std::size_t");
            return true;
        }
...
struct R<Core::Gui::ListView>
{
    ...
    static constexpr std::string_view FullName() { return "Core::Gui::ListView"; }
    static constexpr std::string_view ParentScope() { return "Core::Gui"; }
    ...
    static bool GetField(Core::Gui::ListView& obj, const char* fieldName, const std::function<void(void*, const char*)>& onFound)
    {
        if (!onFound) return false;
...
struct R<Core::Gui::ListModelBased>
{
    ...
    static constexpr std::string_view FullName() { return "Core::Gui::ListModelBased"; }
    static constexpr std::string_view ParentScope() { return "Core::Gui"; }
    static constexpr std::size_t GetFieldNumbers() { return 0; }
    static constexpr std::vector<RClassField> GetFields() {
        return {
    ...
    static bool GetField(Core::Gui::ListModelBased& obj, const char* fieldName, const std::function<void(void*, const char*)>& onFound)
    {
        if (!onFound) return false;
